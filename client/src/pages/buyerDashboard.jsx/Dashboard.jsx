      ...
      <div className='max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-10'>
        ...
        <header className='flex flex-col md:flex-row justify-between items-start md:items-center gap-6'>
          <div className='space-y-1'>
            <div className='flex items-center gap-2'>
              ...
              <Badge variant="secondary" className='rounded-md font-semibold tracking-wider bg-secondary text-secondary-foreground border-none'>
                {profile.role || 'VERIFIED BUYER'}
              </Badge>
            </div>
            <h1 className='text-4xl font-extrabold tracking-tight'>
              Welcome, <span className="text-primary">{profile.fullname.split(' ')[0]}</span>
            </h1>
          </div>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-12 gap-6'>
                ...
                <div className='flex-1 space-y-6'>
                  ...
                  <div className='grid grid-cols-1 xl:grid-cols-2 gap-4'>
                    <div className='flex items-center gap-4 bg-muted/50 p-4 rounded-xl border border-border/50'>
                      ...
                      <span className='text-sm font-semibold truncate'>{profile.email}</span>
                    </div>
                    <div className='flex items-center gap-4 bg-muted/50 p-4 rounded-xl border border-border/50'>
                      <MapPin size={18} className="text-primary" />
                      <span className='text-sm font-semibold'>{profile.address.city}, {profile.address.state}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className='lg:col-span-5 grid grid-cols-2 lg:grid-cols-1 gap-6'>
            {[
              ...
              { label: 'Requests', val: stats.contactCount, icon: MessageSquare, color: 'text-accent', bg: 'bg-accent/10' }
            ].map((stat, i) => (
              <div key={i} className='bg-card p-6 rounded-3xl border border-border shadow-sm flex items-center justify-between group hover:border-primary transition-colors'>
                <div>
                  <p className='text-xs font-bold text-muted-foreground uppercase tracking-widest'>{stat.label}</p>
                  <h3 className='text-4xl font-black mt-1 text-foreground'>{stat.val}</h3>
                </div>
                <div className={`${stat.bg} ${stat.color} p-4 rounded-2xl transition-transform group-hover:scale-105`}>
                  <stat.icon size={28} />
                </div>
              </div>
            ))}
        ...
        <section className='space-y-6'>
          <div className='flex justify-between items-end'>
            <div>
              <h2 className='text-3xl font-black tracking-tight text-foreground'>Recent Interests</h2>
              <p className="text-muted-foreground">Saved products from your network</p>
            </div>
            <Button
              variant='link'
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
            {recentWishlist.length > 0 ? (
                    ...
                    <CardContent className='flex-1 p-6 flex flex-col justify-between'>
                      <div className='space-y-2'>
                        <span className='text-[10px] font-bold text-muted-foreground flex items-center gap-1.5 uppercase'>
                          ...
                          {new Date(item?.item?.expectedHarvest || item?.item?.createdAt).toLocaleDateString()}
                        </span>
                      ...
                      <div className='mt-6 flex justify-between items-end border-t border-border pt-4'>
                        <div>
                          ...
                          <div className='flex items-baseline gap-1'>
                            <span className='text-2xl font-black text-primary'>
                                ₹{(item.item?.expectedPrice?.value || item.item?.price?.value).toLocaleString()}
                            </span>
                            <span className='text-xs font-medium text-muted-foreground uppercase'>
                                /{item.item?.expectedPrice?.unit?.split('_')[1] || 'UNIT'}
                            </span>
                          </div>
                        </div>
                        ...
                        <Button
                          ...
                          onClick={() => navigate(`/buyer/product/${item.item.productType === 'pre-harvest' ? 'pre-harvested' : 'harvested'}/${item.item._id}`)}
                        >
                          Inspect
                        </Button>
                      </div>
              ...
              <div className='col-span-full py-20 bg-muted/30 border-2 border-dashed border-border rounded-[2.5rem] flex flex-col items-center justify-center text-center'>
                ...
                <h3 className="text-lg font-bold text-foreground">No saved items</h3>
                <p className='text-muted-foreground text-sm mt-1 mb-6'>Start exploring the marketplace to fill your wishlist.</p>
                <Button
