      ...
      <div className='max-w-5xl mx-auto space-y-8'>
        ...
        <div className='relative group overflow-hidden rounded-3xl bg-card border shadow-sm p-6 md:p-10'>
          ...
          <div className='flex flex-col md:flex-row items-center gap-8 relative z-10'>
            <div className='relative'>
              <div className='w-32 h-32 rounded-full bg-secondary flex items-center justify-center border-4 border-background shadow-xl overflow-hidden shrink-0'>
                {isLoading ? (
              ...
              <button className='absolute bottom-1 right-1 p-2 bg-primary text-white rounded-full hover:scale-110 transition-transform shadow-lg disabled:opacity-50'>
            ...
            <div className='text-center md:text-left space-y-2 flex-1 min-w-0'>
              <div className='flex flex-wrap items-center justify-center md:justify-start gap-3'>
                {isLoading ? (
                  ...
                  <h1 className='text-3xl font-bold tracking-tight truncate max-w-full'>
                    {farmer?.fullname}
                  </h1>
                )}
                {!isLoading && farmer?.isVarified && (
                  <Badge className='bg-chart-2/20 text-chart-2 border-none hover:bg-chart-2/30 px-3 shrink-0'>
                    <ShieldCheck className='w-3 h-3 mr-1' /> Verified Farmer
                  </Badge>
                )}
              </div>
              {isLoading ? (
                <Skeleton className='h-4 w-32 mx-auto md:mx-0 bg-secondary' />
              ) : (
        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-8'>
          ...
          <div className='space-y-6'>
            {isLoading ? (
              <>
                <Skeleton className='h-44 bg-secondary rounded-2xl' />
